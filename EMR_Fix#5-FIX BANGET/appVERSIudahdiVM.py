...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://emr_user:q4nAyZnt39Fr/FoivFm5D8oKt6w6FsjTt8yLt96lYyk=@192.168.20.2/emr_project_db'
...
def check_login_attempts(username, role_type):
    """
    Cek apakah akun user sedang terkunci (DB-based).
    role_type dipertahankan untuk kompatibilitas API tapi tidak dipakai.
    """
    user = User.query.filter_by(username=username).first()
    if not user:
        return True, None  # Username tidak ada, biarkan handler login yg reject

    if user.account_locked_until:
        if datetime.now() < user.account_locked_until:
            seconds_left = (user.account_locked_until - datetime.now()).total_seconds()
            minutes_left = max(1, int(seconds_left / 60) + 1)
            return False, f"Akun terkunci karena terlalu banyak percobaan gagal. Coba lagi dalam {minutes_left} menit."
        else:
            # Lockout sudah expire, reset otomatis
...
def record_failed_login(username, role_type):
    """
    Catat percobaan login gagal ke DB.
    Returns True jika akun terkunci, False jika belum.
    """
    user = User.query.filter_by(username=username).first()
    if not user:
        return False  # Username tidak ada, tidak ada yang perlu dikunci

    user.failed_login_attempts = (user.failed_login_attempts or 0) + 1
    user.last_failed_login = datetime.now()

    if user.failed_login_attempts >= 8:
        # 5 menit — sinkron dengan Flask-Limiter window
...
def reset_login_attempts(username, role_type):
    """Reset percobaan login setelah sukses (DB-based)."""
    user = User.query.filter_by(username=username).first()
    if user:
        user.failed_login_attempts = 0
        user.account_locked_until = None
...
def staff_login():
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            ...
            if is_locked:
                flash('Terlalu banyak percobaan gagal. Akun terkunci selama 5 menit.', 'danger')
                catat_log(f'Login staff gagal (akun terkunci): {username}')
            else:
                fresh_user = User.query.filter_by(username=username).first()
                attempts_so_far = fresh_user.failed_login_attempts if fresh_user else 0
                remaining = max(0, 8 - attempts_so_far)
